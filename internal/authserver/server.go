...
// Credentials holds the API keys submitted via the form.
...
<head>
    ...
    <title>Connect to Brandfetch</title>
    ...
    <style>
        :root {
            ...
            --yellow: #FFDA00;
            --navy: #0F172A;
            --gradient-pro: linear-gradient(135deg, #C377FF 0%, #2B75E2 100%);
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .api-section:focus-within {
            border-color: var(--border-focus);
            box-shadow: 0 0 0 3px rgba(195, 119, 255, 0.1);
        }

        .api-section.has-value {
        ...
        .api-icon.logo-api {
            background: linear-gradient(135deg, #2B75E2 0%, #60A5FA 100%);
        }
        ...
        input[type="password"]:focus {
            outline: none;
            border-color: var(--border-focus);
            box-shadow: 0 0 0 3px rgba(195, 119, 255, 0.08);
        }
...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <form method="POST" action="/auth" id="authForm">
                <div class="api-section" id="logoSection">
                    <div class="api-header">
                        <div class="api-icon logo-api">
                        ...
                        <div class="api-info">
                            ...
                            <p class="api-desc">High quota for logo lookups and search</p>
                            <div class="api-commands">
                                <span class="cmd-tag">logo</span>
                    ...
                    <input
                        ...
                        id="client_id"
                        name="client_id"
                        placeholder="Enter Logo API key"
                        autocomplete="off"
                    >
                </div>

                <div class="api-section" id="brandSection">
                    <div class="api-header">
                        <div class="api-icon brand-api">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div class="api-info">
                            ...
                            <p class="api-desc">Full brand data with colors, fonts, and assets</p>
                            <div class="api-commands">
                                <span class="cmd-tag">brand</span>
                                <span class="cmd-tag">colors</span>
                                <span class="cmd-tag">fonts</span>
                            </div>
                        </div>
                    </div>
                    <input
                        ...
                        id="api_key"
                        name="api_key"
                        placeholder="Enter Brand API key"
                        autocomplete="off"
                    >
                </div>

                <button type="submit" class="submit-btn" id="submitBtn" disabled>Save Credentials</button>

                <p class="validation-hint">Enter at least one API key to continue</p>
            </form>

            <div class="footer">
                Need an API key? <a href="https://developers.brandfetch.com/register" target="_blank">Register for free</a>
            </div>

            <div class="accent-bar">
                <div class="accent-dot" style="background: #2B75E2"></div>
                <div class="accent-dot" style="background: #C377FF"></div>
                <div class="accent-dot" style="background: #F03063"></div>
                <div class="accent-dot" style="background: #22C55E"></div>
                <div class="accent-dot" style="background: #FFDA00"></div>
            </div>
        </div>

        <a href="https://github.com/salmonumbrella/brandfetch-cli" target="_blank" class="github-link">
    ...
    <script>
        ...
        const validationHint = document.querySelector('.validation-hint');
        const errorEl = document.getElementById('error');

        function updateState() {
            const hasLogo = clientIdInput.value.trim() !== '';
            const hasBrand = apiKeyInput.value.trim() !== '';
            const hasAny = hasLogo || hasBrand;

            submitBtn.disabled = !hasAny;
            validationHint.style.display = hasAny ? 'none' : 'block';

            logoSection.classList.toggle('has-value', hasLogo);
            brandSection.classList.toggle('has-value', hasBrand);
        }

        clientIdInput.addEventListener('input', updateState);
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!hasBrand && hasLogo) {
                apiKeyInput.value = clientIdInput.value.trim();
            } else if (!hasLogo && hasBrand) {
                clientIdInput.value = apiKeyInput.value.trim();
            }
        });
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Connected - Brandfetch CLI</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,400;0,9..40,500;0,9..40,600;0,9..40,700&display=swap" rel="stylesheet">
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .checkmark {
            ...
            animation: checkmark-draw 0.4s ease 0.5s forwards;
        }
...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <p class="message">
                Your API credentials have been securely stored.<br>
                You're ready to start fetching brand data.
            </p>
            ...
            <div class="terminal-hint">
                ...
                <p class="terminal-text">
                    <strong>Return to your terminal<span class="cursor"></span></strong>
...
// Handler handles auth form requests.
type Handler struct {
	resultChan chan<- Credentials
}

// NewHandler creates a new auth handler.
func NewHandler(resultChan chan<- Credentials) *Handler {
	return &Handler{resultChan: resultChan}
}
...
func (h *Handler) showForm(w http.ResponseWriter, _ *http.Request, errorMsg string) {
	w.Header().Set("Content-Type", "text/html")
	// Simple template replacement
	html := formHTML
	if errorMsg != "" {
		html = `<!DOCTYPE html><html><body><div class="error">` + errorMsg + `</div>` + formHTML[strings.Index(formHTML, "<form"):]
	}
...
func (h *Handler) handleSubmit(w http.ResponseWriter, r *http.Request) {
	...
	// Send credentials to channel
	h.resultChan <- Credentials{
		ClientID: clientID,
		APIKey:   apiKey,
	}
...
// NewServer creates a new auth server on an ephemeral port.
...
func (s *Server) URL() string {
	return fmt.Sprintf("http://%s/auth", s.listener.Addr().String())
}
...
// WaitForCredentials waits for credentials to be submitted.
